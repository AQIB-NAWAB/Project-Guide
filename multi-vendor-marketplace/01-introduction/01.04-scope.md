...
Getting this wrong is how projects die at week three: you add Stripe, then webhooks, then a admin dashboard, then realise checkout still doesn't split by vendor. The exclusions below are not limitations — they're **focus**.
...
- **Redis job queue (BullMQ)** — async notifications after checkout and order status changes; separate worker process
- **Deployment** — live HTTPS URL for API, client, and worker; processes survive restart
- **Payment status field** — orders carry status like `payment_pending` or `placed`; no real payment gateway
...
|---|---|---|
| **Real payment processing** (Stripe, Razorpay, etc.) | Payment integration is its own course | Order placed with `payment_pending` or `placed` status; focus on cart split and snapshots |
...
- "I added Stripe instead of checkout" — checkout was the point
...
Customer opens app
  → registers / logs in
  → browses catalogue (filter by category or store)
  → opens product detail
  → adds to cart (repeat from another vendor)
  → opens cart (sees items grouped by store)
  → checks out
  → system creates Order₁ for Vendor A, Order₂ for Vendor B
  → each order line carries snapshot price/name/unit
  → stock decremented atomically
  → cart cleared
  → customer sees confirmation + order history
  → **worker** sends vendor + customer notifications asynchronously (Chapter 22)
  → Vendor A sees Order₁ in their dashboard
  → Vendor B sees Order₂ in their dashboard
```
...
```
Vendor registers / verifies email / logs in
  → no store → "Open your shop" form
  → creates store
  → adds product (name, price, unit, stock, category)
  → uploads photo (Cloudinary URL saved)
  → publishes product
  → product appears in public catalogue
  → vendor sees it in their dashboard table
```
...
```
Vendor A logs in → creates product P
Vendor B logs in → sends PATCH /products/P with Vendor B's token
  → must receive 403 or 404 — never success
Vendor B logs in → sends PATCH /stores/A's store id
  → must receive 403 or 404
```

You'll build explicit tests for this in the authorization chapters. Passing them is part of scope.

### Secondary flow — snapshot immutability (you must test this)

```
Customer checks out product at price ₹100
Vendor updates product price to ₹150
Customer opens order history
  → line item still shows ₹100
```

If it shows ₹150, checkout was built wrong — not a polish bug.
...
| 3 | Catalogue, cart, checkout, order UIs | Customer checkout → two orders if two vendors in cart |
...
**Mistake 1 — Payment first.** Integrating Stripe before checkout splits correctly builds a payment flow on a broken order model. Checkout and snapshots first; payment is out of scope.
...
- **In scope** is the contract — identity, isolation, vendor tools, customer commerce, cache, deploy
- **Out of scope** protects focus — no payments, admin, reviews, or real-time in the core course
