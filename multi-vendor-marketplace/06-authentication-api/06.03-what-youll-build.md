...
A: Namespaces auth routes away from future `/api/products` — mount once in `app.js`.

**Q: Refresh in cookie or JSON body?**  
A: Body OK for 6.18 curl tests; Chapter 7 prefers httpOnly cookie — document both phases.

**Q: Is logout required for gate?**  
A: Optional — refresh revocation is the important logout semantics.

**Q: Where do User models live vs auth routes?**  
A: Models in `modules/users/`; HTTP layer in `modules/auth/` — separation keeps concerns clear.
