...
A: **Signed**, not encrypted — payload is readable; do not put secrets inside.

**Q: Why short access token if refresh exists?**  
A: Limits damage if access token leaks from memory — attacker window is minutes, not days.

**Q: Why not sessions with Express default memory store?**  
A: Fine for tiny apps — does not scale across processes and teaches less about SPA Bearer patterns this course targets.
...
| Module monolith | No separate session Redis until Ch 20 cache |
| Learning goal | Industry-standard JWT + refresh pattern |
| Pair with Ch 7 | 401 refresh loop teaches real client auth |

Redis in Chapter 20 is for **catalogue cache**, not sessions.
